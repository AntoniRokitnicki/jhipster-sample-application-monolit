...
@WithMockUser
@EmbeddedKafka
class JhipsterSampleApplicationMonolitKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/jhipster-sample-application-monolit-kafka/publish?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jhipster-sample-application-monolit-kafka/register"))
            .andExpect(status().isOk())
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/jhipster-sample-application-monolit-kafka/unregister"));
                return;
            }
        }
